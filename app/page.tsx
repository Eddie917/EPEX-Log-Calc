...
import { Trash2, PlusCircle, RefreshCw, Save, Upload, Calculator, Euro, Truck, Download } from "lucide-react";
...
const toNum = (v: number | ""): number => (typeof v === "number" ? v : 0);
...
export default function TransportCostCalculator() {
  ...
  // Vozidlo a ceny (bez defaultov)
  const [consumption, setConsumption] = useState<number | "">(""); // l/100km
  const [fuelPrice, setFuelPrice]     = useState<number | "">(""); // €/l
  const [adbluePct, setAdbluePct]     = useState<number | "">(""); // % z paliva
  ...
  const [perDiem, setPerDiem]         = useState<number | "">(""); // Diéty spolu (€)
  ...
  const perDiemCost  = useMemo(() => toNum(perDiem), [perDiem]); // už je to celková suma
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Calculator size={18}/> Trasa a poplatky</CardTitle>
            </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    step="0.1"
                    min="0"
                    value={prejazdNakladkaKm}
                    onChange={(e) => setPrejazdNakladkaKm(e.target.value === "" ? "" : Number(e.target.value))}
                    className="w-40"
                  />
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {legs.map((leg) => (
                    <div key={leg.id} className="flex items-center gap-2">
                      <Input
                        value={leg.name}
                        onChange={(e) => setLegs(v => v.map(x => x.id === leg.id ? { ...x, name: e.target.value } : x))}
                        className="w-40"
                      />
                      <Input
                        type="number"
                        ...
                        min="0"
                        value={leg.km}
                        onChange={(e) => setLegs(v => v.map(x => x.id === leg.id ? { ...x, km: e.target.value === "" ? "" : Number(e.target.value) } : x))}
                        placeholder="km"
                      />
                      <Button onClick={() => setLegs(v => v.filter(x => x.id !== leg.id))} title="Odstrániť úsek">
                        <Trash2 size={16}/>
                      </Button>
                    </div>
                  ))}
                ...
                <div className="space-y-2">
                  {fees.map((f) => (
                    <div key={f.id} className="flex items-center gap-2">
                      <Input
                        value={f.name}
                        onChange={(e) => setFees(v => v.map(x => x.id === f.id ? { ...x, name: e.target.value } : x))}
                        className="w-40"
                        placeholder="Názov"
                      />
                      <Input
                        ...
                        min="0"
                        value={f.amount}
                        onChange={(e) => setFees(v => v.map(x => x.id === f.id ? { ...x, amount: e.target.value === "" ? "" : Number(e.target.value) } : x))}
                        placeholder="€"
                      />
                      <Button onClick={() => setFees(v => v.filter(x => x.id !== f.id))} title="Odstrániť poplatok">
                        <Trash2 size={16}/>
                      </Button>
                    </div>
                  ))}
          ...
          <Card className="shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Truck size={18}/> Vozidlo a ceny</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-2 gap-4">
              <div>
                <Label>Spotreba (l/100 km)</Label>
                <Input type="number" step="0.1" min="0" value={consumption} onChange={e => setConsumption(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>
              <div>
                <Label>Cena paliva (€/l)</Label>
                <Input type="number" step="0.001" min="0" value={fuelPrice} onChange={e => setFuelPrice(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>
              <div>
                <Label>AdBlue (% z paliva)</Label>
                <Input type="number" step="0.1" min="0" value={adbluePct} onChange={e => setAdbluePct(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>
              <div>
                <Label>Cena AdBlue (€/l)</Label>
                <Input type="number" step="0.01" min="0" value={adbluePrice} onChange={e => setAdbluePrice(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>

              <div>
                <Label>Sadzba vodiča (€/h)</Label>
                <Input type="number" step="0.1" min="0" value={hourlyRate} onChange={e => setHourlyRate(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>
              <div>
                <Label>Jazda (h)</Label>
                <Input type="number" step="0.1" min="0" value={driveHours} onChange={e => setDriveHours(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>

              <div>
                <Label>Diéty (€)</Label>
                <Input type="number" step="0.1" min="0" value={perDiem} onChange={e => setPerDiem(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>

              <div className="col-span-2">
                <Label>Ostatné náklady (€)</Label>
                <Input type="number" step="0.1" min="0" value={otherCost} onChange={e => setOtherCost(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>

              <div>
                <Label>Marža (%)</Label>
                <Input type="number" step="0.1" min="0" value={marginPct} onChange={e => setMarginPct(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>
              <div className="flex items-center gap-3">
                <div>
                  <Label>DPH (%)</Label>
                  <Input type="number" step="1" min="0" value={vatPct} onChange={e => setVatPct(e.target.value === "" ? "" : Number(e.target.value))} />
                </div>
                <div className="pt-6 flex items-center gap-2">
                  <Switch checked={applyVat} onCheckedChange={setApplyVat} />
                  <span className="text-sm">Započítať DPH</span>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Euro size={18}/> Súhrn</CardTitle>
          </CardHeader>
